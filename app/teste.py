...
from municipiosES import get_municipio_coords, COORDENADAS_ESPIRITO_SANTO
...
def render_footer():
    ...
    municipio = list(COORDENADAS_ESPIRITO_SANTO.keys())
    ...
    print(f"Condições Atuais em {next(iter(dados_municipio))}/ES")    
    ...
    if weather_data:
        ...
        # descobre o índice da hora atual
        idx = lista_horas.index(agora_str)

        data_1h = weather_data["data_1h"]

        # 4) Extrair valores usando o mesmo índice
        resultado = {
            "hora": agora_str,
            ...
            "pictocode": data_1h["pictocode"][idx],  # descrição do tempo
        }

        st.markdown(f"### Condições Atuais em {municipio}/ES")
